document.addEventListener('DOMContentLoaded', () => {
    ...
    function setupModalListeners() {
        ...
        document.getElementById('manage-player-form').addEventListener('submit', async (e) => {
            ...
            await saveUserData();
            document.getElementById('manage-player-modal').classList.add('hidden');
            switchView('player-management-view');
        });
    ...
    function getDashboardHTML() {
        ...
        return `<div id="dashboard-content"><div class="view-header"><h2>La mia Top 11 (${formation || 'N/A'})</h2><button id="set-lineup-btn">Schiera Formazione</button></div><div class="football-pitch"><div class="pitch-line center-line"></div><div class="pitch-line center-circle"></div>${playerMarkers}</div></div>`;
    }
    ...
    function getAuctionViewHTML() { return `<div id="auction-content"><div class="view-header"><h3>Modalità Asta</h3><div><input type="checkbox" id="hide-lost-slots-chk"><label for="hide-lost-slots-chk">Nascondi slot persi</label><button id="show-final-squad-btn">Mostra Rosa Finale</button></div></div><div id="auction-list-container"></div></div>`; }
    ...
    function attachAuctionViewListeners() { document.getElementById('show-final-squad-btn').addEventListener('click', renderAndShowFinalSquad); document.getElementById('hide-lost-slots-chk').addEventListener('change', (e) => { hideLostSlots = e.target.checked; renderAuctionView(); }); document.getElementById('auction-list-container').addEventListener('click', (e) => { const action = e.target.dataset.action; const index = e.target.dataset.index; if (action && index) { if (action === 'won') { handlePlayerWon(index); } else if (action === 'lost') { handlePlayerLost(index); } } }); }
    function attachSettingsListeners() { document.getElementById('settings-form').addEventListener('submit', async (e) => { e.preventDefault(); totalCredits = document.getElementById('total-credits').value; roleLimits = { P: document.getElementById('limit-p').value, D: document.getElementById('limit-d').value, C: document.getElementById('limit-c').value, A: document.getElementById('limit-a').value, }; await saveUserData(); alert('Impostazioni salvate!'); }); document.getElementById('reset-auction-btn').addEventListener('click', async () => { if (confirm('Sei sicuro di voler resettare lo stato dell\'asta? Questa azione non può essere annullata.')) { userTopList.forEach(item => { item.auctionStatus = 'pending'; item.activePlan = 'A'; item.finalCost = 0; }); await saveUserData(); alert('Stato dell\'asta resettato!'); } }); }
    ...
    function renderAuctionView() { const container = document.getElementById('auction-list-container'); if(!container) return; let listToRender = hideLostSlots ? userTopList.filter(item => item.auctionStatus !== 'lost') : userTopList; container.innerHTML = listToRender.map((item, index) => { const playerA = item.planA; const playerB = item.planB; const playerC = item.planC; const isWon = item.auctionStatus === 'won'; const isLost = item.auctionStatus === 'lost'; let activePlanHTML = ''; if (!isWon && !isLost) { if (item.activePlan === 'A' && playerA) { activePlanHTML = renderPlayerPlan(playerA, 'A', item.planA_max_budget, index); } else if (item.activePlan === 'B' && playerB) { activePlanHTML = renderPlayerPlan(playerB, 'B', null, index); } else if (item.activePlan === 'C' && playerC) { activePlanHTML = renderPlayerPlan(playerC, 'C', null, index); } } return `<div class="auction-item role-${playerA.ruolo} ${isWon ? 'status-won' : ''} ${isLost ? 'status-lost' : ''}"><h5>Slot ${userTopList.indexOf(item) + 1} (${roleMap[playerA.ruolo]})</h5><div class="player-plan ${item.activePlan !== 'A' && !isWon ? 'lost' : ''}"><b>A:</b> ${playerA.nome} (${item.planA_max_budget} cr)${isWon && item.activePlan === 'A' ? `<strong> - ACQUISTATO a ${item.finalCost}</strong>` : ''}</div>${playerB ? `<div class="player-plan plan-b ${item.activePlan === 'A' || (item.activePlan === 'C' && !isWon) ? 'inactive' : ''} ${item.activePlan !== 'B' && !isWon ? 'lost' : ''}"><b>B:</b> ${playerB.nome}${isWon && item.activePlan === 'B' ? `<strong> - ACQUISTATO a ${item.finalCost}</strong>` : ''}</div>` : ''}${playerC ? `<div class="player-plan plan-c ${item.activePlan !== 'C' || isWon ? 'inactive' : ''} ${item.activePlan !== 'C' && !isWon ? 'lost' : ''}"><b>C:</b> ${playerC.nome}${isWon && item.activePlan === 'C' ? `<strong> - ACQUISTATO a ${item.finalCost}</strong>` : ''}</div>` : ''}<div class="auction-actions">${activePlanHTML}</div></div>`; }).join(''); }
    function renderPlayerPlan(player, plan, budget, index) { if (!player) return ''; return `<div><p><strong>Attuale: ${player.nome}</strong> ${budget ? `(Budget: ${budget})` : ''}</p><input type="number" class="cost-input" data-index="${userTopList.indexOf(player.parentItem)}" placeholder="Costo finale" min="1"><button class="won-btn" data-action="won" data-index="${userTopList.indexOf(player.parentItem)}">Acquistato</button><button class="lost-btn" data-action="lost" data-index="${userTopList.indexOf(player.parentItem)}">Sfumato</button></div>`; }
    async function handlePlayerWon(originalIndex) { const item = userTopList[originalIndex]; const costInput = document.querySelector(`.cost-input[data-index='${originalIndex}']`); const cost = costInput.value; if (!cost || cost < 1) { alert("Inserisci un costo valido."); return; } item.auctionStatus = 'won'; item.finalCost = cost; renderAuctionView(); await saveUserData(); }
    async function handlePlayerLost(originalIndex) { const item = userTopList[originalIndex]; if (item.activePlan === 'A') { item.activePlan = 'B'; } else if (item.activePlan === 'B') { item.activePlan = 'C'; } else { item.auctionStatus = 'lost'; } if ((item.activePlan === 'B' && !item.planB) || (item.activePlan === 'C' && !item.planC)) { item.auctionStatus = 'lost'; } renderAuctionView(); await saveUserData(); }
